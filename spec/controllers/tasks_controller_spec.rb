...
RSpec.describe TasksController, type: :controller do
  ...
  describe "POST /create" do
    context 'with valid params' do
      it 'returns task' do
        authenticate_admin
        post :create, params: { user_id: admin.id, title: "title", description: "xyz", due_date: Date.today, priorities: "high", status: "open"}
  ...
  describe "PUT /update" do
    context 'when admin update task' do
      it " if task updates a successful" do
        authenticate_admin
        put :update, params: { user_id: admin.id, id: task.id, title:"update task"}
    ...
    context 'when manager update task' do
      it " if task updates a successful" do
        ...
        put :update, params: { user_id: manager.id, id: task.id, description: "update description"}
  ...
  describe "GET /show" do
    ...
    context 'When valid id present in params' do
      it "return task" do
        get :show, params: { user_id: admin.id, id: task.id}
        expect(response.status).to eq(200)
      end
    end
  ...
  describe "DELETE /destroy" do
    context 'when valid id presemt in params' do
      it "return task delete" do
        authenticate_admin
        delete :destroy, params: {user_id: 23, id:task.id}
        expect(response.status).to eq(200)
      end
    end
